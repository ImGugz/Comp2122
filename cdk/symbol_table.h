#ifndef __CDK15_SYMBOL_TABLE_H__
#define __CDK15_SYMBOL_TABLE_H__
...
namespace cdk {
  ...
  public:
    ...
    void print_table() {
      std::cout << "-----------------------------------------" << std::endl;
      std::cout << "TABLE WITH " << _contexts.size() << " CONTEXTS" << std::endl;
      for (size_t ix = _contexts.size(); ix > 0; --ix) {
        context_type &ctx = *_contexts[ix - 1];
        std::cout << "CONTEXT " << ix << " WITH " << ctx.size() << " SYMBOLS!" << std::endl;
        for (auto const& x : ctx) {
